...
class KotlinDuplicateFinder {
  ...
  private fun isKotlinSource(
    path: Path,
    includeScripts: Boolean,
  ): Boolean = path.toString().endsWith(".kt") || (includeScripts && path.toString().endsWith(".kts"))
  ...
  private fun collectEntries(
    ...
    element.children.forEach {
      collectEntries(file, text, it, options, normalizer, semanticIndex, childDepth, entries)
    }
  }
  ...
  private fun similarity(
    ...
    val structure = if (fingerprintsUnion.isEmpty()) 0.0 else fingerprintsIntersection.size.toDouble() /
      fingerprintsUnion.size
  ...
  private fun suggestionFor(
    ...
    left.kind == CandidateKind.FUNCTION && right.kind == CandidateKind.FUNCTION &&
      options.compareMode == CompareMode.BODIES ->
      "Consider extracting the repeated body into a private helper, extension function, or higher-order function."

    left.kind == CandidateKind.FUNCTION && right.kind == CandidateKind.FUNCTION &&
      left.semanticFacts.any {
        it.contains("annotations:Composable") || it.contains("annotations:androidx.compose")
      } ->
    ...
    left.kind == right.kind -> "Consider extracting the repeated ${left.kind.cliName.removeSuffix(
      "s",
    )} structure into a Kotlin helper abstraction."
  ...
  private data class Entry(
    ...
    fun overlaps(other: Entry): Boolean = file == other.file && startLine <= other.endLine && other.startLine <= endLine
  }
  ...
  private class PathFilter(
    ...
    fun allows(path: Path): Boolean {
      ...
      val included =
        includes.isEmpty() || includes.any { it.matches(normalized) || it.matches(normalized.fileName) }
