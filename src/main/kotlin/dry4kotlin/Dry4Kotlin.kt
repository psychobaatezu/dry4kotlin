...
object Dry4Kotlin {
  ...
  fun toSarif(
    ...
    append("  \"runs\": [{\n")
    append(
      "    \"tool\": {\"driver\": {\"name\": \"dry4kotlin\", \"rules\": [{\"id\": \"structural-duplicate\", \"name\": \"Structural duplicate\"}]}},\n",
    )
    append("    \"results\": [")
    candidates.forEachIndexed { index, candidate ->
      if (index > 0) append(',')
  ...
  private fun candidateJson(
    ...
    if (options.explain) append(
      ",\n  \"explanations\": [${candidate.explanations.joinToString(",") {
        jsonString(it)
      }}]",
    )
    if (options.showSnippets) {
  ...
  private fun groupJson(
    ...
    if (options.explain) append(
      ",\n  \"explanations\": [${group.explanations.joinToString(",") {
        jsonString(it)
      }}]",
    )
    append("\n")
  ...
  private fun locationJson(
    location: Location,
    options: Options,
  ): String = "{\"file\": ${jsonString(
    displayFile(location.file, options),
  )}, \"startLine\": ${location.startLine}, \"endLine\": ${location.endLine}}"

  private fun sarifLocation(
    location: Location,
    options: Options,
  ): String = "{\"physicalLocation\": {\"artifactLocation\": {\"uri\": ${jsonString(
    displayFile(location.file, options),
  )}}, " +
    "\"region\": {\"startLine\": ${location.startLine}, \"endLine\": ${location.endLine}}}}"

  private fun locationEdn(
    location: Location,
    options: Options,
  ): String = "{:file \"${escape(
    displayFile(location.file, options),
  )}\", :start-line ${location.startLine}, :end-line ${location.endLine}}"

  private fun lineRange(
    location: Location,
    options: Options,
  ): String = "${displayFile(location.file, options)}:${location.startLine}-${location.endLine}"
  ...
  private fun displayFile(
    ...
    return runCatching {
      Path.of("").toAbsolutePath().relativize(path.toAbsolutePath()).toString()
    }.getOrElse { file }
  }
