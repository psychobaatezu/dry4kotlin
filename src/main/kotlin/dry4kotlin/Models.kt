...
data class Candidate(
  ...
  val sizeScore: Double = if (leftNodes == 0 ||
    rightNodes == 0
  ) 0.0 else minOf(leftNodes, rightNodes).toDouble() / maxOf(leftNodes, rightNodes),
  ...
  companion object {
    ...
    fun parse(text: String): CandidateKind {
      ...
      return entries.firstOrNull {
        it.cliName == normalized || it.name.lowercase().replace('_', '-') == normalized
      }
