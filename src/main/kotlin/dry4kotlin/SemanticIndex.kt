...
internal class SemanticIndex private constructor(
    ...
    fun hasIgnoredAnnotation(element: PsiElement, ignored: Set<String>): Boolean {
        ...
        return annotations.any { annotation ->
            annotation in ignored || imports[annotation] in ignored || ignored.any { it.substringAfterLast('.') == annotation }
        }
    }
    ...
    private fun resolveName(element: PsiElement, name: String): String {
        val file = element.containingFile as? KtFile
        val imports = file?.let { fileInfo[it]?.importsByName } ?: emptyMap()
    ...
    companion object {
        ...
        private fun buildFqName(file: KtFile, declaration: KtNamedDeclaration, name: String): String {
